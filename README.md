...
- all the auction participants can send their bids to the smart contract **SC<sub>A</sub>**: in the bidding process, each participant `p` will include an hidden secret `h<sub>p</sub>(s<sub>p</sub>)` which will be used to bind the accounts possessed by each participant on the Algorand and Ethereum blockchain;
- the smart contract **SC<sub>A</sub>** eventually closes the bidding phase and announces the winner;
- the oracle **O** detects the end of the auction phase and sends a transaction to **SC<sub>E</sub>** containing a reference to the hidden secret `h<sub>w</sub>(s<sub>w</sub>)` of the winner `w`.
...
- the seller uses the secret  `s<sub>w</sub>` published by the winner `w` to redeem the transaction in Algo on the Algorand blockchain.
